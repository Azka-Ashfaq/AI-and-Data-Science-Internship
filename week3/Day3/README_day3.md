...
| `guardrail_eval.py` | 1, 5 | 10 adversarial prompts, 18-prompt guardrail test set, automated grounding check (numbers in the answer must trace back to a real tool output, with rounding tolerance), auto-generated report |
...
since models naturally round when speaking). This runs automatically on every response in the
Task 5 evaluation, so a hallucinated number is caught mechanically, not just by eyeballing.
