...
def classify_query(query: str) -> IntentClassification:
    ...
    if has_retrieval_signal:
     ...
     # 4.5 Future-looking question → prediction (needs clarification)
    if re.search(r"\bwill\b", q_lower) and _matches_any(q_lower, FUTURE_MARKERS):
        return IntentClassification(
            intent="prediction",
            confidence=0.7,
            rationale="Future-tense question; routing to prediction for clarification.",
        )
